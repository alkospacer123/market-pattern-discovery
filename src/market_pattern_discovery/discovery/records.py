...
def validate_effect_record(record: dict[str, Any]) -> None:
    """Apply the tracked JSON Schema, then the runtime status semantics."""
    schema_path=Path(__file__).resolve().parents[3]/"schemas"/"discovery_effect_record_v1.json"
    validator=Draft202012Validator(json.loads(schema_path.read_text()))
    try:
        validator.validate(record)
    except ValidationError as exc:
        raise ValueError(f"invalid discovery effect record: {exc.message}") from exc
