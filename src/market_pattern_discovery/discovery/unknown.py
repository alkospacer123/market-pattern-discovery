...
def _load_window_sources(
    ...
    # The signed DISCOVERY guard is mandatory in addition to generic ingestion.
    ...
    if not paths:
        raise FileNotFoundError(f"no current Finam sources for {instrument} {timeframe}")
    if any("2025" in str(path) for path in paths):
        raise PermissionError("2025 TRUE OOS source path is sealed")
    loaded = stitch_finam(paths, instrument, timeframe)
    frame = loaded.frame.loc[(loaded.frame.open_time >= start) & (loaded.frame.open_time < end)].copy()
    if frame.empty:
    ...
    frame["moscow_trading_date"] = frame.open_time.dt.tz_convert("Europe/Moscow").dt.date
    return frame.reset_index(drop=True), loaded.provenance
