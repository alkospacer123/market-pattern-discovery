...
def main(argv=None) -> int:
    ...
    freeze = sub.add_parser("freeze"); freeze.add_argument("--data-root", type=Path, required=True); freeze.add_argument("--output-root", type=Path, required=True); freeze.add_argument("--passport-root", type=Path, default=Path("bbw_system/config/instruments")); freeze.add_argument("--symbols", nargs="+", help="optional canonical instrument scope"); freeze.add_argument("--start-date", help="inclusive raw timestamp date (YYYY-MM-DD)"); freeze.add_argument("--end-date", help="inclusive raw timestamp date (YYYY-MM-DD)")
    args = parser.parse_args(argv)
    if args.command == "audit": return run_audit(args.data_root, args.output, args.passport_root)
    if args.command == "freeze": return run_freeze(args.data_root, args.output_root, args.passport_root, args.symbols, args.start_date, args.end_date)
