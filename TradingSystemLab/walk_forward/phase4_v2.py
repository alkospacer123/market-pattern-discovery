"""Phase 4 walk-forward evidence for the four immutable v2 candidates.

This is a calendar-coverage adaptation of :mod:`walk_forward.phase4`; it has no
...
from ..baseline_v2 import DATA_ROOT, FROZEN_TICK_SIZE, INSTRUMENTS, load_development
...
from ..robustness.phase3_v2 import EXPECTED_IDS, load_frozen_registry
from ..strategies.trend.T3_MTF_Trend import T3MTFTrend, T3Parameters
from ..baseline_v2 import four_bar_context
from dataclasses import replace

OUTPUT_ROOT = Path("TradingSystemLab/results/walk_forward_v2")
REGISTRY_PATH = Path("TradingSystemLab/results/phase3_candidate_freeze/candidate_registry.json")
ROBUSTNESS_ROOT = Path("TradingSystemLab/results/robustness_v2")
FREEZE_REFERENCE_COMMIT = "ff0db54b35544086f4265c303b1018d57d3849e7"
ROBUSTNESS_REFERENCE_COMMIT = "d89d0f1f7ba3259e0f08e43a0832bd16f4d15f6b"
STUDIES = (("T2", "M30"), ("T2", "H1"), ("T3", "M30"), ("T3", "H1"))
ROBUSTNESS = {("T2", "M30"): "ROBUST_READY", ("T2", "H1"): "BORDERLINE",
              ("T3", "M30"): "ROBUST_READY", ("T3", "H1"): "ROBUST_READY"}
SCHEDULE = (
    ("WF01", "2020-01-01", "2023-12-31 23:59:59", "2024-01-01", "2024-03-31 23:59:59"),
    ("WF02", "2020-01-01", "2024-03-31 23:59:59", "2024-04-01", "2024-06-30 23:59:59"),
    ("WF03", "2020-01-01", "2024-06-30 23:59:59", "2024-07-01", "2024-09-30 23:59:59"),
    ("WF04", "2020-01-01", "2024-09-30 23:59:59", "2024-10-01", "2024-12-31 23:59:59"),
)
...
def _verify_before_data(registry_path: Path) -> tuple[list[dict[str, Any]], str]:
    ...
    if robust.get("status") != "PHASE_3_ROBUSTNESS_COMPLETE" or robust.get("classifications") != {
...
def run(data_root: Path = DATA_ROOT, output: Path = OUTPUT_ROOT,
        ...
        for symbol in INSTRUMENTS:
            ...
            coverage[timeframe][symbol] = {
                ...
                "actual_history_limitation": ("source begins after Development start" if
                    frame.index.min() > pd.Timestamp("2020-01-07", tz=frame.index.tz) else None),
    ...
    for item in candidates:
        ...
        metrics = {"candidate_id": item["candidate_id"], "strategy": strategy, "timeframe": timeframe,
            ...
            "frozen_parameter_hash": item["parameter_hash"], "aggregate": {"C1": aggregate},
    ...
    status = ("PHASE_4_WALK_FORWARD_COMPLETE" if all(v == "WALK_FORWARD_PASS" for v in verdicts.values())
    ...
    manifest = {"phase": "PHASE_4_WALK_FORWARD", "methodological_source": "original H1 Phase 4",
        ...
        "development_coverage": ["2020-01-01", "2024-12-31"],
        ...
        "true_oos_start": "2025-01-01", "true_oos_blocked": True, "true_oos_read": False,
    ...
    lines = ["# TradingSystemLab v2 — Phase 4 Walk Forward", "",
    ...
    lines += ["", "Development = 2020–2024; four expanding quarterly tests in 2024; C1 only.",
