"""Causal M15 expanding-window validation of the frozen T2/T3 candidates.
...
from ..timeframe_validation.m15_baseline import (INSTRUMENTS, TRADE_COLUMNS,
    causal_h1_context, load_m15_development)

OUTPUT = Path("TradingSystemLab/results/walk_forward/M15")
...
STATUS = "PHASE_M15_WALK_FORWARD_COMPLETE"
...
_H1_CONTEXT_CACHE: dict[int, pd.DataFrame] = {}
...
def reject_true_oos(values: Any) -> None:
    ...
    if len(stamps) and (stamps >= TRUE_OOS_START.tz_convert("UTC")).any():
        raise ValueError("M15_TRUE_OOS_ACCESS_REJECTED")
...
def generate_folds(common_start: pd.Timestamp, coverage_end_exclusive: pd.Timestamp) -> list[dict[str, Any]]:
    ...
    if coverage_end_exclusive > TRUE_OOS_START:
        raise ValueError("M15_FOLD_TOUCHES_TRUE_OOS")
    ...
    if any(a["test_end"] > b["test_start"] for a, b in zip(folds, folds[1:])):
        raise RuntimeError("M15_OVERLAPPING_FOLDS")
    return folds
...
def _canonical_sources(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        alias = row["alias"]
        item = {"instrument": row["instrument"], "alias": alias, "files": row["files"]}
        if alias in seen and seen[alias] != item:
            raise RuntimeError("M15_SOURCE_SNAPSHOT_INTERNALLY_INCONSISTENT")
        seen[alias] = item
...
def validate_prerequisites(baseline: Path = BASELINE, optimization: Path = OPTIMIZATION,
    ...
    expected = (("M15_BASELINE", "PHASE_M15_BASELINE_COMPLETE"),
                ("M15_OPTIMIZATION", "PHASE_M15_OPTIMIZATION_COMPLETE"),
                ("M15_ROBUSTNESS", "PHASE_M15_ROBUSTNESS_COMPLETE"))
    for manifest, (phase, status) in zip(manifests, expected):
        if (manifest.get("phase"), manifest.get("status"), manifest.get("timeframe"),
                manifest.get("development_period")) != (phase, status, "M15", DEVELOPMENT_PERIOD):
            raise RuntimeError(f"{phase}_PROVENANCE_INVALID")
    bm, om, rm = manifests
    snapshots = [_canonical_sources(m["source_data_hashes"]) for m in manifests]
    if snapshots[0] != snapshots[1] or snapshots[1] != snapshots[2]:
        raise RuntimeError("M15_PREREQUISITE_SOURCE_SNAPSHOT_MISMATCH")
    registries = {}
    for key, identity in EXPECTED.items():
        path = optimization / key / "candidate_registry.json"
        registry = json.loads(path.read_text(encoding="utf-8"))
        if tuple(registry.get(x) for x in ("candidate_id", "configuration_id", "parameter_hash")) != identity:
        ...
        if registry.get("strategy_hash") != STRATEGY_SHA256[key] or registry.get("source_hashes") != snapshots[0]:
...
def _normalize(raw: pd.DataFrame, key: str, alias: str, fold: str) -> pd.DataFrame:
    ...
    frame["strategy"], frame["timeframe"], frame["fold_id"] = key, "M15", fold
    frame["net_R"] = frame.gross_R.astype(float) - 2 * COST_TICKS_PER_SIDE / frame.initial_risk_ticks.astype(float)
    frame["trade_id"] = [f"{key}-M15-{fold}-{alias}-{i:06d}" for i in range(1, len(frame) + 1)]
    return frame
...
def execute_window(key: str, registry: dict, alias: str, candles: pd.DataFrame,
    ...
    if key == "T2":
        ...
        context = _H1_CONTEXT_CACHE.get(id(candles))
        if context is None:
            context = causal_h1_context(candles)
            _H1_CONTEXT_CACHE[id(candles)] = context
    ...
    if len(result):
        ...
        if not ((entries >= start.tz_convert("UTC")) & (entries < end.tz_convert("UTC"))).all():
            raise RuntimeError("M15_FOLD_ENTRY_OWNERSHIP_VIOLATION")
...
def _run_candidate(key: str, registry: dict, loaded: dict, folds: list[dict], target: Path) -> dict:
    ...
    if stitched.trade_id.duplicated().any() or stitched[["instrument", "entry_time"]].duplicated().any():
        raise RuntimeError("M15_DUPLICATE_FORWARD_ENTRY")
    ...
    (target / "final_report.md").write_text(
        f"# {registry['candidate_id']} M15 Walk Forward\n\n"
...
def run(data_root: Path = APPROVED_DATA_ROOT, output: Path = OUTPUT,
    ...
    expected_sources = _canonical_sources(bm["source_data_hashes"])
    ...
    if len(folds) != 3:
        raise RuntimeError("M15_EXPECTED_THREE_COMPLETE_FOLDS")
    ...
    if before != after:
        raise RuntimeError("M15_PROTECTED_ARTIFACT_MUTATION")
    ...
    manifest = {"phase": "M15_WALK_FORWARD", "status": STATUS, "timeframe": "M15",
        ...
        "common_coverage": {"first_close": common_start.isoformat(), "last_close": common_last.isoformat(),
        ...
        "flat_initialization_per_fold": True, "causal_h1_context": True,
        "protected_artifact_hashes": after, **flags}
    _json(output / "manifest.json", manifest)
    lines = ["# M15 Causal Walk Forward Validation", "", "Frozen candidates validated independently; this is not a ranking.", "",
