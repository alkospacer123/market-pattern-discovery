"""M5 deterministic, development-only bounded optimization following the H1 methodology.

The design is intentionally small: the M5 baseline plus every declared
...
from ..timeframe_validation.m5_baseline import INSTRUMENTS, _execute, load_m5_development

OUTPUT = Path("TradingSystemLab/results/timeframe_optimization/M5")
BASELINE_ROOT = Path("TradingSystemLab/results/timeframe_validation/M5")
...
PROTECTED = (BASELINE_ROOT, *H1_ARTIFACTS)
BASELINES = {
...
def baseline_candidates(path: Path = BASELINE_ROOT / "manifest.json") -> dict[str, dict[str, Any]]:
    """Verify embedded frozen inputs against the M5 baseline manifest.

    Parameters are embedded to ensure this phase never opens an H1 optimization
    or robustness artifact.
    """
    manifest = json.loads(path.read_text(encoding="utf-8"))
    if manifest.get("phase") != "M5_BASELINE" or manifest.get("timeframe") != "M5":
    ...
    if any(hashes.get(key) != stable_hash(BASELINES[key]["parameters"]) for key in BASELINES):
        raise RuntimeError("M5_BASELINE_PARAMETER_HASH_MISMATCH")
    return BASELINES
...
def bounded_design(key: str, baseline: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(design) > MAX_CONFIGURATIONS:
        raise RuntimeError("M5_CONFIGURATION_LIMIT_EXCEEDED")
    return design
...
def _evaluate(task: tuple[str, dict, str]) -> dict[str, Any]:
    ...
    if _WORKER_DATA is None:
        raise RuntimeError("M5_WORKER_DATA_NOT_INITIALIZED")
...
def _run_strategy(key: str, baseline_row: dict, loaded: dict, output: Path,
    ...
    for number, config in enumerate(configs):
        cid = f"{key}-M5-{number:04d}-{stable_hash(config)[:12]}"
    ...
    candidate_id = f"{key}_M5_candidate_v1"; parameter_hash = stable_hash({n: selected[n] for n in RANGES[key]})
    registry = {"candidate_id": candidate_id, "strategy": key, "timeframe": "M5",
...
def run(data_root: Path = APPROVED_DATA_ROOT, output: Path = OUTPUT) -> dict[str, Any]:
    """Run separate T2 and T3 M5 searches and atomically verify old artifacts."""
    ...
    manifest = {"phase": "M5_OPTIMIZATION", "status": "PHASE_M5_OPTIMIZATION_COMPLETE", "timeframe": "M5",
        "development_period": ["2023-01-01", "2024-12-31"], "instruments": [x[0] for x in INSTRUMENTS],
        "strategies": ["T2_M5_candidate_v1", "T3_M5_candidate_v1"], "source_data_hashes": sources,
    ...
    lines = ["# M5 Bounded Optimization", "", "Independent bounded T2 and T3 development searches; no cross-timeframe ranking or winner selection.", "",
    ...
    lines += ["", "2025+ TRUE OOS, walk-forward, portfolio construction, and H1 artifacts were not accessed.", "",
              "PHASE_M5_OPTIMIZATION_COMPLETE", ""]
    (output / "m5_optimization_report.md").write_text("\n".join(lines), encoding="utf-8")
