"""TradingSystemLab v2 Phase 2B: T3 M30/H1 bounded OAT optimization."""
...
from .phase32 import _normalize_backtester

OUTPUT_ROOT = Path("TradingSystemLab/results/optimization_v2/T3")
TIMEFRAMES = ("M30", "H1")
STRATEGY = "T3"
...
PHASE2A_T2_COMMIT = "a252f076a09495d3093854c8509b1551701ba2b4"
...
def run_study(timeframe: str, data_root: Path, output: Path) -> dict[str, Any]:
    ...
    (output / "final_report.md").write_text(
        ...
        "All stable configurations are reported without ranking or candidate selection. T3 used separate causal four-bar context. TRUE OOS was not read.\n")
...
def run(data_root: Path = DATA_ROOT, output: Path = OUTPUT_ROOT) -> dict[str, Any]:
    ...
    (output / "manifest.json").write_text(json.dumps({"phase": "PHASE_2_OPTIMIZATION", "strategy": "T3",
        "timeframes": list(TIMEFRAMES), "studies": 2, "configurations_per_study": 22,
        "total_configuration_studies": 44, "candidate_selection": False, "true_oos_blocked": True},
    ...
    lines += [f"| {r['timeframe']} | 22 | {r['overall']} | {r['robust_configurations']} | {r['recommendation']} |" for r in studies]
    lines += ["", "T3 Phase 2B only. Robustness, walk-forward, TRUE OOS, portfolio selection, candidate freezing, and Phase 7 were not executed.", ""]
    (output / "T3_Optimization_Report.md").write_text("\n".join(lines))
    return {"status": "T3_PHASE_2B_EXECUTED", "studies": studies}
