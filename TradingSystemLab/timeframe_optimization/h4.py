"""Strict H4 bounded-OAT optimization derived from H1 Phase 3.2.

This module deliberately reuses the audited H4 baseline loader and execution
...
PROTECTED = tuple(Path(p) for p in (
    ...
    "TradingSystemLab/results/timeframe_optimization/M1",
    "TradingSystemLab/results/timeframe_optimization/M5",
    "TradingSystemLab/results/timeframe_optimization/M15",
    "TradingSystemLab/results/timeframe_optimization/M30"))
...
def protected_snapshot() -> dict[str, str]:
    """Hash protected trees and each frozen strategy source explicitly."""
...
def _metrics(configuration_id: str, trades: pd.DataFrame) -> dict[str, Any]:
    ...
    scopes: dict[str, pd.Series] = {alias: trades.symbol.eq(alias) for alias in ("Si", "CNY")}
    years = pd.to_datetime(trades.exit_time, utc=True).dt.year if len(trades) else pd.Series(dtype=int)
    for year in (2023, 2024):
        scopes[str(year)] = years.eq(year)
    for direction in ("LONG", "SHORT"):
        scopes[direction] = trades.direction.eq(direction)
    for label, mask in scopes.items():
        part = stats(values[mask])
        row.update({f"{label}_trades": part["trades"], f"{label}_PF_C1": part["PF_R"],
                    f"{label}_expectancy_C1": part["expectancy"], f"{label}_net_R_C1": part["net_R"],
                    f"{label}_max_DD_C1": part["max_DD_R"],
                    f"{label}_recovery_factor_C1": part["recovery_factor"],
                    f"{label}_win_rate_C1": part["winrate"]})
...
def _execute_task(task: tuple[str, dict[str, Any], str]) -> dict[str, Any]:
    key, config, cid = task
    if _DATA is None:
        raise RuntimeError("H4_DATA_NOT_INITIALIZED")
...
def _validate_prerequisites(data_root: Path) -> tuple[dict, dict[str, pd.DataFrame]]:
    ...
    required = (manifest.get("status") == baseline.STATUS and manifest.get("timeframe") == "H4" and
    ...
    for key, path in zip(("T2", "T3"), FROZEN_STRATEGY_FILES):
        if _sha(path) != STRATEGY_SHA256[key]:
            raise RuntimeError(f"{key}_STRATEGY_HASH_MISMATCH")
...
def _reports(key: str, rows: list[dict], results: list[dict], target: Path,
    ...
    experiment = {"phase": PHASE, "optimization_type": "BOUNDED_OAT",
        ...
        "timeframe": "H4", "development_period": DEVELOPMENT_PERIOD, "true_oos_blocked": True,
    ...
    manifest = {"phase": PHASE, "strategy": key, "status": STATUS, "timeframe": "H4",
...
def run(data_root: Path = APPROVED_DATA_ROOT, output: Path = OUTPUT) -> dict[str, Any]:
    ...
    protected_before.update({str(path): _sha(path) for path in FROZEN_STRATEGY_FILES})
    ...
    protected_after.update({str(path): _sha(path) for path in FROZEN_STRATEGY_FILES})
    if protected_before != protected_after:
        raise RuntimeError("PROTECTED_RESEARCH_ARTIFACT_MUTATION")
    top = {"phase": PHASE, "status": STATUS, "timeframe": "H4", "development_period": DEVELOPMENT_PERIOD,
        "true_oos_cutoff": TRUE_OOS_CUTOFF, "h4_baseline_manifest_sha256": _sha(BASELINE_MANIFEST),
        "h4_baseline_trade_ledger_sha256": {key: _sha(baseline.OUTPUT / key / "trades.csv") for key in ("T2", "T3")},
        ...
        "methodological_source": "H1_PHASE_3_2", "parameter_spaces": SPACES,
        ...
        "optimization": True, "selection": False, "winner_selection": False,
        "robustness": False, "walk_forward": False, "true_oos_blocked": True,
        "parameter_space_expansion": False, "strategy_change": False,
        "cost_model": "H1_C1", "cost_scenarios": ["C1"], "ticks_per_side": 1,
        "round_trip_ticks": 2, "additional_slippage_ticks": 0}
    _json(output / "manifest.json", top)
    lines = ["# H4 Bounded OAT Optimization", "", "Original H1 Phase 3.2 methodology; full 2023–2024 causal H4 execution data.", ""]
    ...
    (output / "h4_optimization_report.md").write_text("\n".join(lines), encoding="utf-8")
