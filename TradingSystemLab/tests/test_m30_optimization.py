"""Contract tests for the strict M30 bounded-OAT phase."""
...
from TradingSystemLab.timeframe_optimization import m30
from TradingSystemLab.timeframe_validation import m30_baseline as baseline


def test_baseline_prerequisite_and_locked_period():
    manifest = json.loads(m30.BASELINE_MANIFEST.read_text())
    assert manifest["status"] == "PHASE_M30_BASELINE_COMPLETE"
    assert manifest["timeframe"] == "M30"
...
def test_exact_h1_phase32_spaces():
    assert m30.SPACES["T2"] == {"ema_fast": [15, 20, 25, 30], "ema_trend": [40, 50, 60],
    ...
    assert m30.SPACES["T3"] == {"ema_period": [50, 75, 100, 150, 200],
...
def test_oat_design_counts_identity_and_determinism():
    for key, count in (("T2", 19), ("T3", 22)):
        first, second = m30.configuration_rows(key), m30.configuration_rows(key)
        ...
        assert next({name: row[name] for name in m30.SPACES[key]} for row in first
                    if row["baseline_configuration"]) == center
        for row in first:
            differences = sum(row[name] != center[name] for name in m30.SPACES[key])
            assert differences == (0 if row["baseline_configuration"] else 1)
            assert row["configuration_id"].startswith(f"{key}-M30-")


def test_immediate_neighbor_is_one_adjacent_factor_only():
    configs = m30.bounded_design("T2")
    center = configs.index(baseline.EXPECTED["T2"]["parameters"])
    neighbors = m30.immediate_neighbors(configs, center, "T2")
    assert neighbors
    for index in neighbors:
        differing = [name for name in m30.SPACES["T2"] if configs[index][name] != configs[center][name]]
        assert len(differing) == 1
        name = differing[0]
        values = m30.SPACES["T2"][name]
...
def test_exact_plateau_classification_threshold():
    ...
    assert m30.classify(.10, [.07, .13, -.1]) == (2, 2, "ROBUST_PLATEAU")
    assert m30.classify(.10, [.07, .20]) == (2, 1, "LOCAL_SPIKE")
    assert m30.classify(0.0, [.001, .002]) == (2, 0, "NO_EDGE")
    # absolute floor controls near zero.
    assert m30.classify(.001, [.009, .010]) == (2, 2, "ROBUST_PLATEAU")
...
def test_execution_contract_and_no_later_phase_flags():
    ...
    assert "T2TrendPullback" in source and "causal_four_m30_context" in source
    assert baseline.causal_four_m30_context.__doc__.startswith("Use the canonical H1 four-bar")
    module_source = m30.Path(m30.__file__).read_text()
...
def test_committed_outputs_have_required_deterministic_tree():
    ...
    for key in ("T2", "T3"):
        assert {p.name for p in (m30.OUTPUT / key).iterdir()} == required
    manifest = json.loads((m30.OUTPUT / "manifest.json").read_text())
