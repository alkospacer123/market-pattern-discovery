...
from TradingSystemLab.audit_perpetual_v3_phase2b_t3 import ROOT, audit
...
MUTATIONS = {
    ...
    "parameter_outside_space": lambda root: csv_mutation(root, "M30/parameters.csv", lambda frame: frame.loc.__setitem__((0, "ema_period"), 999)),
    "multi_factor": lambda root: csv_mutation(root, "M30/parameters.csv", lambda frame: (frame.__setitem__("ema_period", frame.ema_period.mask(frame.index == 0, 50)), frame.__setitem__("breakout_period", frame.breakout_period.mask(frame.index == 0, 55)))),
    ...
    "context_contract": lambda root: json_mutation(root, "M30/manifest.json", "context", "shared frame"),
    "strategy_hash": lambda root: json_mutation(root, "M30/manifest.json", "frozen_strategy_hash", "0" * 64),
