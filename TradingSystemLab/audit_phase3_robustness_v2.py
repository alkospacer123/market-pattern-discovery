"""Independent fail-closed audit of v2 Phase 3 robustness artifacts."""
...
from TradingSystemLab.robustness.phase3_v2 import (
...
def audit(root: Path = ROOT) -> dict[str, Any]:
    ...
    _require(_unchanged("TradingSystemLab/results/baseline_v2"), "PHASE1_TREE_CHANGED")
    _require(_unchanged("TradingSystemLab/results/optimization_v2"), "PHASE2_TREE_CHANGED")
    _require(_unchanged("TradingSystemLab/results/phase3_candidate_freeze"), "FREEZE_TREE_CHANGED")
    root_manifest = json.loads((root / "validation_manifest.json").read_text())
    _require(root_manifest["status"] in {"PENDING_AUDIT", "PHASE_3_ROBUSTNESS_COMPLETE"}, "ROOT_STATUS_INVALID")
    ...
    _require(root_manifest["development_period"] == ["2020-01-01", "2024-12-31"] and
...
def main() -> None:
    ...
    manifest["status"] = "PHASE_3_ROBUSTNESS_COMPLETE"
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True, allow_nan=False) + "\n")
    (ROOT / "Phase_3_Robustness_Audit_Report.md").write_text(
        "# Phase 3 Robustness v2 Independent Audit\n\n**PASS** — all 28 mandatory invariants passed. "
        "The four frozen studies reconciled to Phase 2; Phase 1, Phase 2, and freeze evidence remained unchanged.\n",
        encoding="utf-8")
