"""One-shot TRUE OOS validation for the frozen M30 T2/T3 candidates.
...
from ..timeframe_validation.m30_baseline import INSTRUMENTS, TRADE_COLUMNS, causal_four_m30_context

TIMEFRAME = "M30"
PHASE = "M30_TRUE_OOS"
STATUS = "PHASE_M30_TRUE_OOS_COMPLETE"
DEVELOPMENT_PERIOD = ["2023-01-01", "2024-12-31"]
TRUE_OOS_PERIOD = ["2025-01-01", None]
OUTPUT = Path("TradingSystemLab/results/true_oos_validation/M30")
FROZEN_REGISTRY = Path("TradingSystemLab/results/timeframe_analysis/M30_ROBUSTNESS/candidate_registry.json")
WALK_FORWARD = Path("TradingSystemLab/results/walk_forward/M30")
...
EVALUATED_CANDIDATES = {"T2": "T2_M30_candidate_v1", "T3": "T3_M30_candidate_v1"}
PRE_OOS_VERDICTS = {"T2": "WALK_FORWARD_BORDERLINE", "T3": "WALK_FORWARD_PASS"}
...
def reject_pre_oos(values: Any) -> None:
    ...
    if len(stamps) and (stamps < TRUE_OOS_START.tz_convert("UTC")).any():
        raise RuntimeError("M30_TRUE_OOS_PRE_BOUNDARY_ACCESS")
...
def _assert_execution_only() -> None:
    ...
    if forbidden:
        raise RuntimeError("M30_TRUE_OOS_FORBIDDEN_API_CALL")
...
def load_frozen_registry(registry_root: Path = FROZEN_REGISTRY,
    ...
    if wf.get("status") != "PHASE_M30_WALK_FORWARD_COMPLETE" or wf.get("development_period") != DEVELOPMENT_PERIOD:
        raise RuntimeError("M30_WALK_FORWARD_PROVENANCE_INVALID")
    ...
    for key in ("T2", "T3"):
        ...
        identity = (row.get("candidate_id"), row.get("optimization_configuration_id"), row.get("parameter_hash"))
        if identity != (expected_id, configuration, parameter_hash):
            raise RuntimeError(f"{key}_FROZEN_CANDIDATE_IDENTITY_MISMATCH")
        if row.get("parent_candidate_id") != ALLOWED_CANDIDATES[key] or row.get("selection_locked_before_validation") is not True:
        ...
        if (wf.get("candidate_ids", {}).get(key), wf.get("configuration_ids", {}).get(key),
                wf.get("parameter_hashes", {}).get(key), wf.get("parent_candidate_ids", {}).get(key)) != (
                expected_id, configuration, parameter_hash, ALLOWED_CANDIDATES[key]):
        ...
        row["configuration_id"] = row["optimization_configuration_id"]
        row["baseline_candidate_id"] = row["parent_candidate_id"]
        result[key] = row
    return result
...
def load_true_oos(data_root: Path, alias: str) -> tuple[pd.DataFrame, list[Path]]:
    ...
    frame = loader.close_index(raw, "30min")
...
class _OOST2(T2TrendPullback):
    ...
    def _validate(frame: pd.DataFrame) -> None:
        ...
        if not {"Open", "High", "Low", "Close"}.issubset(frame.columns):
            raise ValueError("M30 OHLC columns are required")
    ...
    for instrument, alias in INSTRUMENTS:
        ...
        if key == "T2":
            ...
            if len(context) and any(context.index > candles.index[-1]):
                raise RuntimeError("M30_TRUE_OOS_LOOK_AHEAD")
        ...
        frame["trade_id"] = [f"{key}-M30-TRUE-OOS-{alias}-{i:06d}" for i in range(1, len(frame) + 1)]
        pieces.append(frame)
...
def _reports(target: Path, key: str, registry: dict, trades: pd.DataFrame) -> dict:
    ...
    (target / "final_report.md").write_text(
        f"# {registry['candidate_id']} — M30 TRUE OOS\n\n**Classification: {classification}**\n\n"
...
def run(data_root: Path = APPROVED_DATA_ROOT, output: Path = OUTPUT,
    ...
    prerequisites = ((BASELINE, "PHASE_M30_BASELINE_COMPLETE"),
                     (OPTIMIZATION, "PHASE_M30_OPTIMIZATION_COMPLETE"),
                     (ROBUSTNESS, "PHASE_M30_ROBUSTNESS_COMPLETE"),
                     (Path(walk_forward), "PHASE_M30_WALK_FORWARD_COMPLETE"))
    for root, expected in prerequisites:
        if json.loads((root / "manifest.json").read_text(encoding="utf-8")).get("status") != expected:
            raise RuntimeError(f"M30_PREREQUISITE_INVALID:{root}")
    ...
    if before != {str(path): hash_tree(path) for path in protected}:
        raise RuntimeError("M30_TRUE_OOS_FROZEN_ARTIFACT_MUTATION")
    comparison = []
    for key in ("T2", "T3"):
