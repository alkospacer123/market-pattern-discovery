"""One-shot Phase 5 TRUE OOS evaluation of the four frozen v2 identities."""
...
from ..baseline_v2 import DATA_ROOT, FROZEN_TICK_SIZE, INSTRUMENTS, four_bar_context
...
from ..robustness.phase3_v2 import load_frozen_registry
...
OUTPUT_ROOT = Path("TradingSystemLab/results/true_oos_v2")
REGISTRY_PATH = Path("TradingSystemLab/results/phase3_candidate_freeze/candidate_registry.json")
WALK_FORWARD_ROOT = Path("TradingSystemLab/results/walk_forward_v2")
...
FREEZE_COMMIT = "ff0db54b35544086f4265c303b1018d57d3849e7"
ROBUSTNESS_COMMIT = "d89d0f1f7ba3259e0f08e43a0832bd16f4d15f6b"
WALK_FORWARD_COMMIT = "0b0027665fdcc0f847b6b9a10cb5928fcfd2d553"
...
def reports(item: Mapping[str,Any], trades: pd.DataFrame, target: Path) -> tuple[dict[str,Any],str]:
    ...
    exits=pd.to_datetime(trades.exit_time,utc=True); years=sorted(exits.dt.year.unique()); quarters=exits.dt.to_period("Q").astype(str)
    yearly=_group(trades.assign(year=exits.dt.year),"year",years); quarterly=_group(trades.assign(quarter=quarters),"quarter",sorted(quarters.unique()))
    instruments=_group(trades,"symbol",list(INSTRUMENTS)); directions=_group(trades,"direction",["LONG","SHORT"])
    for name,rows in (("yearly_report.csv",yearly),("quarterly_report.csv",quarterly),("instrument_report.csv",instruments),("direction_report.csv",directions)): _csv(target/name,rows)
...
def run(data_root: Path=DATA_ROOT,output: Path=OUTPUT_ROOT) -> dict[str,Any]:
    ...
    wf=json.loads((WALK_FORWARD_ROOT/"summary/manifest.json").read_text()); expected={f"{s}/{t}":"WALK_FORWARD_BORDERLINE" for s,t in STUDIES}
    ...
    for item in candidates:
        ...
        phase2=pd.read_csv(Path("TradingSystemLab/results/optimization_v2")/s/tf/"results.csv").set_index("configuration_id").loc[item["phase2_configuration_id"]]
        ...
        manifest={"phase":"PHASE_5_TRUE_OOS","methodological_source":"original H1 Phase 5","strategy":s,"timeframe":tf,"candidate_id":item["candidate_id"],"phase2_configuration_id":item["phase2_configuration_id"],"frozen_parameter_hash":item["parameter_hash"],"frozen_strategy_source_hash":STRATEGY_HASHES[s],"freeze_reference_commit":FREEZE_COMMIT,"robustness_reference_commit":ROBUSTNESS_COMMIT,"walk_forward_reference_commit":WALK_FORWARD_COMMIT,"true_oos_start":"2025-01-01","actual_first_admitted_oos_close":first.isoformat(),"actual_last_admitted_oos_close":last.isoformat(),"admitted_oos_rows":sum(coverage[tf][x]["admitted_oos_rows"] for x in INSTRUMENTS),"development_rows_admitted":0,"start_state":"FLAT","cold_start":True,"C1_only":True,"normalized_research_tick":FROZEN_TICK_SIZE,"parameters_frozen":True,"optimization":False,"ranking":False,"candidate_replacement":False,"development_state_reused":False,"phase7_mtf_research":False,"execution_context":"none" if s=="T2" else f"four completed non-overlapping {tf} bars; local-day reset","classification":verdict}; _json(target/"manifest.json",manifest)
