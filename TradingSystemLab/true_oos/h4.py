"""One-shot TRUE OOS validation for the frozen H4 T2/T3 candidates.
...
from ..timeframe_validation.h4_baseline import INSTRUMENTS, TRADE_COLUMNS, causal_h4, causal_d1

TIMEFRAME = "H4"
PHASE = "H4_TRUE_OOS"
STATUS = "PHASE_H4_TRUE_OOS_COMPLETE"
DEVELOPMENT_PERIOD = ["2023-01-01", "2024-12-31"]
TRUE_OOS_PERIOD = ["2025-01-01", None]
OUTPUT = Path("TradingSystemLab/results/true_oos_validation/H4")
FROZEN_REGISTRY = Path("TradingSystemLab/results/timeframe_robustness/H4/candidate_registry.json")
WALK_FORWARD = Path("TradingSystemLab/results/walk_forward/H4")
BASELINE = Path("TradingSystemLab/results/timeframe_validation/H4")
ROBUSTNESS = Path("TradingSystemLab/results/timeframe_robustness/H4")
...
EVALUATED_CANDIDATES = {"T2": "T2_H4_candidate_v1", "T3": "T3_H4_candidate_v1"}
PRE_OOS_VERDICTS = {"T2": "WALK_FORWARD_BORDERLINE", "T3": "WALK_FORWARD_BORDERLINE"}
BOOTSTRAP_ITERATIONS = 10_000
BOOTSTRAP_SEED = 5102025
EXPECTED_REGISTRY = {
    "T2": ("T2_H4_candidate_v1", "T2-H4-0008-2b0494cdd24b", "2b0494cdd24bfbfa8ce7d657d6f83d1408f07b9f565d38d093dec5b4856e3c00"),
    "T3": ("T3_H4_candidate_v1", "T3-H4-0003-9b1e60957d91", "9b1e60957d918a086d58a9a721faa60c5dbd66d73721b08c733be460c930215f"),
}
...
WALK_FORWARD_COMMIT = "81a44b82a9bbcaa18ddb42f8b0cb9f9237c03022"
...
def reject_pre_oos(values: Any) -> None:
    ...
    if len(stamps) and (stamps < TRUE_OOS_START.tz_convert("UTC")).any():
        raise RuntimeError("H4_TRUE_OOS_PRE_BOUNDARY_ACCESS")
...
def _assert_execution_only() -> None:
    ...
    if forbidden:
        raise RuntimeError("H4_TRUE_OOS_FORBIDDEN_API_CALL")
...
def load_frozen_registry(registry_root: Path = FROZEN_REGISTRY,
    ...
    provenance = wf.get("provenance", {})
    commits = {"baseline": BASELINE_COMMIT, "optimization": OPTIMIZATION_COMMIT,
               "robustness": ROBUSTNESS_COMMIT}
    if (wf.get("status") != "PHASE_H4_WALK_FORWARD_BORDERLINE" or
            wf.get("verdicts") != PRE_OOS_VERDICTS or
            any(provenance.get(name, {}).get("commit") != commit for name, commit in commits.items())):
        raise RuntimeError("H4_WALK_FORWARD_PROVENANCE_INVALID")
    registry_path = Path(registry_root)
    rows = {row["strategy"]: row for row in json.loads(registry_path.read_text(encoding="utf-8"))}
    result: dict[str, dict] = {}
    for key in ("T2", "T3"):
        row = dict(rows[key])
        expected_id, configuration, parameter_hash = EXPECTED_REGISTRY[key]
        identity = (row.get("candidate_id"), row.get("source_h4_optimization_configuration_id"), row.get("parameter_hash"))
        if identity != (expected_id, configuration, parameter_hash):
            raise RuntimeError(f"{key}_FROZEN_CANDIDATE_IDENTITY_MISMATCH")
        if row.get("source_h1_candidate_id") != ALLOWED_CANDIDATES[key] or row.get("selection_locked_before_validation") is not True:
        ...
        if (wf.get("candidate_ids", {}).get(key), wf.get("parameter_hashes", {}).get(key)) != (
                expected_id, parameter_hash):
            raise RuntimeError(f"{key}_WALK_FORWARD_IDENTITY_MISMATCH")
        aggregate = json.loads((Path(walk_forward) / key / "metrics.json").read_text(encoding="utf-8"))
        ...
        row["configuration_id"] = row["source_h4_optimization_configuration_id"]
        row["baseline_candidate_id"] = row["source_h1_candidate_id"]
        result[key] = row
    return result
...
class _OOST2(T2TrendPullback):
    ...
    def _validate(frame: pd.DataFrame) -> None:
        ...
        if not {"Open", "High", "Low", "Close"}.issubset(frame.columns):
            raise ValueError("H4 OHLC columns are required")
...
def execute_candidate(key: str, registry: Mapping[str, Any], loaded: Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for instrument, alias in INSTRUMENTS:
        ...
        frame["holding_time"] = (pd.to_datetime(frame.exit_time, utc=True) -
                                 pd.to_datetime(frame.entry_time, utc=True)).astype(str)
        frame["trade_id"] = [f"{key}-H4-TRUE-OOS-{alias}-{i:06d}" for i in range(1, len(frame) + 1)]
        pieces.append(frame[["trade_id", "strategy", "instrument", "symbol", "timeframe", "direction",
            "entry_time", "entry_price", "initial_stop", "initial_risk_ticks", "exit_time", "exit_price",
            "exit_reason", "gross_R", "net_R", "MAE_R", "MFE_R", "holding_time"]])
...
def _metric(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    return {"trades": item["trades"], "PF": finite(item["PF_R"]),
        ...
        "average_win_R": finite(item["average_win_R"]), "average_loss_R": finite(item["average_loss_R"]),
        "max_winning_streak": item["max_winning_streak"], "max_losing_streak": item["max_losing_streak"],
        "average_holding_minutes": finite(holding.mean()), "median_holding_minutes": finite(holding.median())}
...
def _reports(target: Path, key: str, registry: dict, trades: pd.DataFrame) -> dict:
    ...
    months = exits.dt.strftime("%Y-%m")
    monthly_frame = trades.assign(month=months)
    _csv(target / "monthly_report.csv", [{"month": month, **_metric(monthly_frame.loc[monthly_frame.month.eq(month)])}
    ...
    for label, sample in (("ALL", trades), ("WINNERS", trades.loc[trades.net_R > 0]), ("LOSERS", trades.loc[trades.net_R <= 0])):
        excursions.append({"group": label, "trades": len(sample), **{f"{name}_{stat}":
            (finite(sample[name].astype(float).quantile({"p05": .05, "p95": .95}[stat]))
             if stat.startswith("p") and len(sample) else
             finite(getattr(sample[name].astype(float), stat)()) if len(sample) else None)
            for name in ("MAE_R", "MFE_R") for stat in ("mean", "median", "p05", "p95")}})
    ...
    (target / "final_report.md").write_text(
        f"# {registry['candidate_id']} — H4 TRUE OOS\n\n**Classification: {classification}**\n\n"
        ...
        f"Trades: {metrics['trades']}; PF: {metrics['PF']}; expectancy: {metrics['expectancy_R']} R; "
        f"net: {metrics['net_R']} R; max DD: {metrics['max_drawdown_R']} R; recovery: {metrics['recovery_factor']}.\n\n"
        f"Bootstrap P(mean R > 0): {metrics['bootstrap_probability_mean_R_gt_0']}; positive observed quarters: "
        f"{metrics['positive_quarters']}/{metrics['observed_quarters']}; net R without top five: "
        f"{metrics['net_R_without_top5']}.\n\n"
        "Instrument expectancies: " + ", ".join(f"{row['instrument']}={row['expectancy_R']}" for row in instruments) +
        ". Direction expectancies: " + ", ".join(f"{row['direction']}={row['expectancy_R']}" for row in directions) + ".\n",
        encoding="utf-8")
    return metrics
...
def protected_roots(output: Path) -> tuple[Path, ...]:
    """Return every completed result tree except this phase's new output."""
    results = Path("TradingSystemLab/results")
    roots = [path for path in results.iterdir() if path.is_dir() and path.name != "true_oos_validation"]
    true_oos = results / "true_oos_validation"
    if true_oos.exists():
        roots.extend(path for path in true_oos.iterdir()
                     if path.is_dir() and path.resolve() != Path(output).resolve())
...
def run(data_root: Path = APPROVED_DATA_ROOT, output: Path = OUTPUT,
    ...
    prerequisites = ((BASELINE, "PHASE_H4_BASELINE_COMPLETE"),
                     (OPTIMIZATION, "PHASE_H4_OPTIMIZATION_COMPLETE"),
                     (ROBUSTNESS, "PHASE_H4_ROBUSTNESS_COMPLETE"),
                     (Path(walk_forward), "PHASE_H4_WALK_FORWARD_BORDERLINE"))
    for root, expected in prerequisites:
        if json.loads((root / "manifest.json").read_text(encoding="utf-8")).get("status") != expected:
            raise RuntimeError(f"H4_PREREQUISITE_INVALID:{root}")
    registries = load_frozen_registry(Path(registry_root), Path(walk_forward))
    protected = protected_roots(Path(output))
    before = {str(path): hash_tree(path) for path in protected}
    strategy_before = {str(path): _sha(path) for path in (
        Path("TradingSystemLab/strategies/trend/T2_Trend_Pullback.py"),
        Path("TradingSystemLab/strategies/trend/T3_MTF_Trend.py"))}
    loaded, coverage = {}, []
    for instrument, alias in INSTRUMENTS:
        ...
        coverage.append({"instrument": instrument, "alias": alias, "bars": len(frame),
            ...
            "years_present": sorted(set(frame.index.year.astype(int))),
            "files": [{"instrument": instrument, "alias": alias, "filename": path.name,
                       "year": int(path.name.split("_")[2]), "sha256": _sha(path)} for path in paths]})
    output = Path(output)
    if output.exists(): shutil.rmtree(output)
    output.mkdir(parents=True)
    summaries = {key: _reports(output / key, key, registries[key], execute_candidate(key, registries[key], loaded))
                 for key in ("T2", "T3")}
    if (before != {str(path): hash_tree(path) for path in protected} or
            strategy_before != {path: _sha(Path(path)) for path in strategy_before}):
        raise RuntimeError("H4_TRUE_OOS_FROZEN_ARTIFACT_MUTATION")
    comparison = []
    for key in ("T2", "T3"):
        development_row = pd.read_csv(ROBUSTNESS / key / "baseline_vs_candidate.csv").query("version == 'candidate'").iloc[0]
        development = {"trades": int(development_row.trades), "PF": development_row.PF,
            "expectancy_R": development_row.expectancy, "net_R": development_row.net_R,
            "max_drawdown_R": development_row.max_DD}
        wf_metrics = json.loads((Path(walk_forward) / key / "metrics.json").read_text(encoding="utf-8"))["aggregate"]
        forward = {"trades": wf_metrics["trades"], "PF": wf_metrics["PF_C1"],
            "expectancy_R": wf_metrics["expectancy_C1"], "net_R": wf_metrics["net_R_C1"],
            "max_drawdown_R": wf_metrics["max_DD_C1"]}
        oos = summaries[key]
    ...
    source_count = sum(len(row["files"]) for row in coverage)
    (output / "final_true_oos_report.md").write_text(
        "# H4 TRUE OOS Validation\n\n## Provenance\n\n"
        f"Method: H1 Phase 5; sources: {source_count}; first close: {min(row['first_close'] for row in coverage)}; "
        f"last close: {max(row['last_close'] for row in coverage)}; development rows read: 0.\n\n## Results\n\n" + "\n".join(
            f"- **{registries[k]['candidate_id']}: {summaries[k]['classification']}** (pre-OOS: {PRE_OOS_VERDICTS[k]})"
            for k in ("T2", "T3")) +
        "\n\nSee `comparison.csv` for development vs corrected Walk Forward vs TRUE OOS metrics.\n\n"
        "## Safety\n\nCold OOS-only start; no development warm-up; C1 only; no optimization, ranking, "
        "candidate reselection, parameter change, or strategy change. Protected prior research was unchanged.\n\n"
        "PHASE_H4_TRUE_OOS_COMPLETE\n",
        encoding="utf-8")
    manifest = {"phase": PHASE, "status": STATUS, "methodological_source": "H1_PHASE_5", "timeframe": TIMEFRAME,
        "development_period": DEVELOPMENT_PERIOD,
        "true_oos_period": [TRUE_OOS_PERIOD[0], max(row["last_close"] for row in coverage)],
        "true_oos_start": "2025-01-01", "true_oos_end": max(row["last_close"] for row in coverage),
        ...
        "candidate_registry_hash": _sha(Path(registry_root)),
        "baseline_manifest_hash": _sha(BASELINE / "manifest.json"),
        "optimization_manifest_hash": _sha(OPTIMIZATION / "manifest.json"),
        "robustness_manifest_hash": _sha(ROBUSTNESS / "manifest.json"),
        "walk_forward_manifest_hash": _sha(Path(walk_forward) / "manifest.json"),
        "pre_oos_robustness_classifications": {"T2": "BORDERLINE", "T3": "BORDERLINE"},
        "pre_true_oos_walk_forward_verdicts": PRE_OOS_VERDICTS, "strategy_hashes": STRATEGY_SHA256,
        "prior_merge_commits": {"baseline": BASELINE_COMMIT, "optimization": OPTIMIZATION_COMMIT,
            "robustness": ROBUSTNESS_COMMIT, "walk_forward": WALK_FORWARD_COMMIT},
        ...
        "internal_resets": 0, "resets": 0, "retraining": False, "optimization": False, "ranking": False,
        "selection": False, "candidate_selection": False, "parameters_frozen": True,
        "parameter_change": False, "strategy_change": False, "filter_search": False,
        "walk_forward": False, "portfolio": False, "development_warmup": False,
        "cold_start": True, "true_oos_isolated": True, "development_rows_read": 0,
        "development_files_read": 0, "deterministic": True,
        "cost_model": "H1_C1", "cost_scenarios": ["C1"], "ticks_per_side": 1,
        "cost_details": {"name": "H1_C1", "cost_ticks_per_side": COST_TICKS_PER_SIDE,
        ...
        "protected_artifact_hashes_before": before, "protected_strategy_hashes_before": strategy_before,
