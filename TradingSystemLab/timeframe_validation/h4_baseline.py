"""Standalone H4 baseline for the frozen H1 T2/T3 candidates.

This workflow is deliberately descriptive.  It reads only explicitly named
2023/2024 H1 files, constructs causal H4/D1 inputs, and exposes no optimization,
...
from ..multitimeframe.phase71 import (APPROVED_DATA_ROOT, COST_TICKS_PER_SIDE,
    DEVELOPMENT_START, STRATEGY_SHA256, TRUE_OOS_START, TimeframeAdapter,
...
TIMEFRAME = "H4"
PHASE = "H4_BASELINE"
STATUS = "PHASE_H4_BASELINE_COMPLETE"
OUTPUT = Path("TradingSystemLab/results/timeframe_validation/H4")
...
def load_h1_development(data_root: Path, alias: str) -> tuple[pd.DataFrame | None, list[Path]]:
    ...
    frame = loader.close_index(loader.load_csv(paths), "1h")
...
def causal_h4(h1: pd.DataFrame) -> pd.DataFrame:
    return DataLoader.h4_from_h1(h1)


def causal_d1(h1: pd.DataFrame) -> pd.DataFrame:
    """Completed Moscow-day candles, labelled only at their final H1 close."""
    return TimeframeAdapter("D1").execution(h1)
...
def _execute(key: str, parameters: dict, alias: str, h1: pd.DataFrame) -> pd.DataFrame:
    ...
    if key == "T2":
        ...
        raw = Backtester(FixedRiskPortfolio(), cost_ticks_per_side=0, tick_size=tick).run(
            T3MTFTrend(params), alias, execution, causal_d1(h1)).trades
    ...
    frame["trade_id"] = [f"{key}-H4-{alias}-{n:06d}" for n in range(1, len(frame) + 1)]
...
def run(data_root: Path = APPROVED_DATA_ROOT, output: Path = OUTPUT) -> dict[str, Any]:
    ...
    for instrument, alias in INSTRUMENTS:
        ...
        coverage.append({"instrument": instrument, "alias": alias, "source_bar_count": len(frame),
                         "h4_bar_count": len(h4), "first_source_close": frame.index.min().isoformat(),
                         "last_source_close": frame.index.max().isoformat(), "first_h4_close": h4.index.min().isoformat(),
                         "last_h4_close": h4.index.max().isoformat(), "files": files,
    ...
    lines = ["# H4 Baseline Research", "", "Frozen H1 candidates on causal H4 execution candles over full 2023–2024 development data.", "",
    ...
    lines += ["", "T2 ran directly on H4. T3 used completed Moscow-day D1 context with no future fill.",
              "H1 C1: one tick per side; no additional slippage. No optimization, ranking, or selection.", "", STATUS, ""]
    (output / "h4_baseline_report.md").write_text("\n".join(lines), encoding="utf-8")
