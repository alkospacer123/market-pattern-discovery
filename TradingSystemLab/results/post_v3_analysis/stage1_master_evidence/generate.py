    ...
    for path in study.trade_files:
        ...
        with path.open(newline="", encoding="utf-8") as fh:
            for row in csv.DictReader(fh):
                ...
                value = r.get("net_R") or r.get("net_R_C1") or r.get("R_result")
                if value in (None, ""):
                    # Early T3 ledgers store the same immutable outcome as
                    # gross/profit R plus a separately recorded cost in R.
                    value = float(r.get("profit_R") or r["gross_R"]) - float(r.get("cost_R") or 0)
                r["_r"] = float(value)
...
def build():
    ss = studies(); trade_map = {s.key: read_trades(s) for s in ss}
    inventory = []
    for s in ss:
    ...
    for s in ss:
        rows=trade_map[s.key]; base=stats(rows); meta=source_metrics(s)
        ...
        for r in rows:
            months[r["_time"].strftime("%Y-%m")].append(r); years[str(r["_time"].year)].append(r)
            quarters[f'{r["_time"].year}-Q{(r["_time"].month-1)//3+1}'].append(r)
        monthly_vals=[]
        ...
        for m, by_year in sorted(moy.items()):
            vals=list(by_year.values()); relevant=[r for r in rows if r["_time"].month==m]
        ...
        agg=meta.get("aggregate", {}); conc=sorted((r["_r"] for r in rows if r["_r"]>0), reverse=True)
        rec=dict(generation=s.generation,strategy=s.strategy,timeframe=s.timeframe,futures_type=UNIVERSES[s.generation][0],instrument_universe=UNIVERSES[s.generation][1],
            lifecycle_stage=s.stage,candidate_id=meta.get("candidate_id",NA),classification=meta.get("classification",CLASSIFICATIONS.get((s.generation,s.strategy,s.timeframe),NA) if s.stage=="true_oos" else NA),
            development_start=min(r["_time"] for r in rows).date() if s.stage=="development" else NA,development_end=max(r["_time"] for r in rows).date() if s.stage=="development" else NA,
            wf_start=min(r["_time"] for r in rows).date() if s.stage=="walk_forward" else NA,wf_end=max(r["_time"] for r in rows).date() if s.stage=="walk_forward" else NA,
            true_oos_start=min(r["_time"] for r in rows).date() if s.stage=="true_oos" else NA,true_oos_end=max(r["_time"] for r in rows).date() if s.stage=="true_oos" else NA,
            ...
            trade_frequency=base["trades"] / max(1,(max(r["_time"] for r in rows)-min(r["_time"] for r in rows)).days/365.2425),
        ...
        for n in (1,3,5): rec[f"net_R_without_top{n}"]=base["net_R"]-sum(conc[:n]); rec[f"top{n}_positive_R_share"]=sum(conc[:n])/sum(conc) if conc else NA
        for prefix, stage in (("development_or_phase2","development"),("WF","walk_forward"),("TRUE_OOS","true_oos")):
            other=stats(trade_map[(s.generation,stage,s.strategy,s.timeframe)])
            rec[f"{prefix}_PF"]=other["PF"]; rec[f"{prefix}_expectancy"]=other["expectancy_R"]; rec[f"{prefix}_DD"]=other["max_drawdown_R"]
        master.append(rec)
...
def report(ss, inventory, master, instruments, directions, months):
    unavailable="bootstrap and gate fields for older/development stages; historical candidate IDs and verdicts where source metrics omit them; top-1/top-3 source concentration reports (derived descriptive shares are supplied); robustness-stage trade ledgers for all generations"
...
def manifest(ss, inventory):
    ...
    obj={"status":"POST_V3_STAGE_1_MASTER_EVIDENCE_COMPLETE","generation_labels":["v1","v2","v3"],
         ...
         "artifacts":{p.name:sha(p) for p in outputs},"controls":{"no_strategy_execution":True,"no_optimization":True,"no_ranking":True,"no_candidate_replacement":True,"no_trade_modification":True}}
