#!/usr/bin/env python3
"""Build the Stage 3A.3 v2 normalized trade partitions (normalization only)."""
...
OUT = Path(os.environ.get("STAGE3A3_OUTPUT_DIR", STAGE))
FINAL_3A1 = "POST_V3_STAGE_3A1_SCHEMA_PROVENANCE_AUDIT_PASSED"
FINAL_3A2 = "POST_V3_STAGE_3A2_V1_NORMALIZATION_AUDIT_PASSED"
EXPECTED_SCHEMAS = {"V2_T2_BASELINE", "V2_T2_WF", "V2_T3_BASELINE", "V2_T3_WF", "V2_TRUE_OOS"}
EXPECTED_FAMILIES = {"baseline_v2", "walk_forward_v2", "true_oos_v2"}
EXPECTED_COUNTS = {
...
PARTITIONS = {
    "baseline": "normalized_trades_v2_baseline.csv",
    "walk_forward": "normalized_trades_v2_walk_forward.csv",
    "true_oos": "normalized_trades_v2_true_oos.csv",
}
FIELDS = [
...
def verify_prerequisites() -> tuple[dict, dict, list[dict[str, str]], dict[str, dict[str, str]]]:
    ...
    inventory = [row for row in read_csv(STAGE / "trade_source_inventory.csv") if row["generation"] == "v2"]
    registry = {row["source_schema_id"]: row for row in read_csv(STAGE / "trade_schema_registry.csv") if row["generation"] == "v2"}
...
def main() -> None:
    ...
    for item in inventory:
        ...
        for row_number, raw in enumerate(source_rows, 2):
            ...
            if instrument not in {"Si", "CNY", "GD", "BR", "MIX", "NG"}:
                raise RuntimeError("invalid quarterly instrument")
            ...
            key = "|".join(("v2", item["lifecycle_stage"], item["strategy"], item["timeframe"], instrument, item["source_path"], tail))
    ...
    if len(normalized) != 6954:
        raise RuntimeError(f"expected 6954 rows, found {len(normalized)}")
    groups = Counter((r["lifecycle_stage"], r["strategy"], r["timeframe"]) for r in normalized)
    if dict(groups) != EXPECTED_COUNTS:
        raise RuntimeError("v2 study counts differ from contract")
    ...
    expected = {(r["lifecycle_stage"], r["strategy"], r["timeframe"]): r for r in read_csv(STAGE / "study_expected_counts.csv") if r["generation"] == "v2"}
    ...
    recon_path = OUT / "v2_normalization_reconciliation.csv"
    ...
    report = """# Stage 3A.3 — v2 Normalization Report
...
"""
    report_path = OUT / "Stage_3A3_v2_Normalization_Report.md"
    report_path.write_text(report, encoding="utf-8")
    output_hashes = {p.name: sha(p) for p in (*partition_paths, recon_path, report_path)}
    dump_json(OUT / "manifest_stage3a3_v2.json", {
        "status": "POST_V3_STAGE_3A3_V2_NORMALIZATION_COMPLETE", "audit_status": "PENDING_INDEPENDENT_AUDIT",
        ...
        "no_v1_rows": True, "no_v3_rows": True, "no_analysis": True, "no_optimization": True, "no_ranking": True, "no_rule_testing": True,
    })
    print("POST_V3_STAGE_3A3_V2_NORMALIZATION_COMPLETE; audit_status = PENDING_INDEPENDENT_AUDIT")
